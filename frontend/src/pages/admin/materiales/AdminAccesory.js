...
export default function AdminAccesory() {
    // --- NUEVO: comprobación síncrona de rol ---
    ...
    // --- comprobación SÍNCRONA de rol (después de declarar hooks) ---
    ...
    const fetchResults = async (searchQuery = "") => {
        ...
        try {
            ...
            return data;
        } catch (err) { console.error("Error fetching Accessory:", err); setResults([]); return []; } finally { setIsLoading(false); }
    };
    const fetchAll = async () => {
        ...
        try {
            ...
            setResults(normalizeItems(data));
            return data;
        } catch (err) { console.error("[fetchAllAccessories] Error:", err); setResults([]); return []; } finally { setIsLoading(false); }
    };
    ...
    const handleCreate = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const resp = await axios.post(`${API_URL}/api/accessories`, form, { headers: { Authorization: `Bearer ${token}` } });
            const createdId = resp?.data ?? resp?.data?.result ?? resp?.data?.id ?? null;
            let id = createdId;
            if (typeof id === "object" && id !== null) id = id.id ?? id._id ?? null;
            if (typeof id === "string" && /^[0-9]+$/.test(id)) id = Number(id);
            const newItem = { id: id ?? undefined, name: form.name, price: form.price };
        ...
        } catch (err) { console.error(err); toast.error("Error al crear accesorio."); } finally { setModalSubmitting(false); }
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.put(`${API_URL}/api/accessories/${selected.id}`, { name: form.name, price: form.price }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const handleDelete = async (t) => {
        ...
        } catch (err) { console.error(err); toast.error("Error al eliminar accesorio."); } finally { setDeletingIds(prev => ({ ...prev, [key]: false })); }
    };
                ...
                <div style={{ marginTop: 8 }}>
                    ...
                    <form className="search-form" onSubmit={handleSearch}>
                        ...
                        <div className="search-actions">
                            ...
                            <button type="button" onClick={openCreateModal} disabled={isLoading}>Cargar accesorio</button>
                    ...
                    {showModal && (
                        <div className="modal-overlay" onClick={closeModal}>
                            <div className="modal" onClick={e => e.stopPropagation()}>
                                <h3>{selected ? "Actualizar accesorio" : "Crear accesorio"}</h3>
