...
export default function AdminCoating() {
	...
	const handleCreate = async ()=> {
		...
		try {
			const token = localStorage.getItem("token");
			const resp = await axios.post(`${API_URL}/api/coating`, form, { headers: { Authorization: `Bearer ${token}` } });
	...
	const handleUpdate = async ()=> {
		...
		try {
			const token = localStorage.getItem("token");
			await axios.put(`${API_URL}/api/coating/${selected.id}`, { name: form.name, price: form.price }, { headers: { Authorization: `Bearer ${token}` } });
								...
								return (
									<div key={itemKey} className="results-row">
										...
										<div className="col actions">
											<button className="btn update" onClick={()=>{ setSelected(t); setForm({name:t.name, price:t.price}); setShowModal(true); }}>Actualizar</button>
