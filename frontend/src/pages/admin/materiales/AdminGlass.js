...
export default function AdminGlass() {
	...
	const handleCreate = async () => {
		...
		try {
			...
			const resp = await axios.post(`${API_URL}/api/glass-types`, payload, { headers: { Authorization: `Bearer ${token}` } });
	...
	const handleUpdate = async () => {
		...
		try {
			const token = localStorage.getItem("token");
			const payload = { name: form.name, price: form.price, description: form.description };
			await axios.put(`${API_URL}/api/glass-types/${selected.id}`, payload, { headers: { Authorization: `Bearer ${token}` } });
										...
										return (
											<div key={itemKey} className="results-row">
												...
												<div className="col-actions">
													<button className="btn update" onClick={() => { setSelected(t); setForm({ name: t.name || "", price: t.price || 0, description: t.description ?? "" }); setShowModal(true); }}>Actualizar</button>
